...
class iSTFT_module_1_8_mod(nn.Module):
    
    def __init__(self, n_fft, hop_length, win_length, length, center = False, window = None, mode = 'real_imag',device = 'cpu'):
        super(iSTFT_module_1_8_mod, self).__init__()
        self.mode = mode
        self.n_fft = n_fft
        self.hop_length = hop_length
        self.win_length = win_length
    ...
    def forward(self, x):
        ...
        spec_complex = x[:,0] + x[:,1] * 1j
        frame_chunks = torch.fft.irfft(spec_complex)
        zero_padding = (self.n_fft - self.win_length) // 2
        frame_chunks = frame_chunks[:,:,zero_padding:-zero_padding]
        frame_chunks = frame_chunks * self.window
        if self.center:
            s = torch.nn.Fold(output_size=[length,1], kernel_size=[self.win_length,1],stride=[self.hop_length,1])(frame_chunks.permute([0,2,1]))[:,0,self.padding_num:-self.padding_num,0] 
        else:
...
class iSTFT_module_1_7_mod(nn.Module):
    
    def __init__(self, n_fft, hop_length, win_length, length, center = False, window = None, mode = 'real_imag',device = 'cpu'):
        super(iSTFT_module_1_7_mod, self).__init__()
        self.mode = mode
        self.n_fft = n_fft
        self.hop_length = hop_length
        self.win_length = win_length
    ...
    def forward(self, x):
        ...
        length = self.win_length + self.hop_length * (x.shape[-2] - 1)
        # bs,T,F,2 --> bs,T,F*2
        frame_chunks = torch.irfft(x.permute([0,2,3,1]),signal_ndim=1, signal_sizes=[self.n_fft])
        zero_padding = (self.n_fft - self.win_length) // 2
        frame_chunks = frame_chunks[:,:,zero_padding:-zero_padding]
        frame_chunks = frame_chunks * self.window
        if self.center:
